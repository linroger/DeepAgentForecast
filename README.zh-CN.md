...
> 📖 **文档：**[DeepWiki — 交互式 AI 生成文档](https://deepwiki.com/linroger/DeepAgentForecast) · [`DRF_ARCHITECTURE.md`](DRF_ARCHITECTURE.md) —— 基于代码的完整系统架构图（每个阶段、每个服务、每条跨阶段契约，附 `file:line` 引用）。
...
- **多智能体群体模拟**：数以百计的 LLM 人格在模拟的 Twitter + Reddit 上互动；每轮即一个日历单位的模拟时间，涌现的群体动态为预测提供输入。
