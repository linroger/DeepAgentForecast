...
def test_actor_projection_uses_summary_but_not_global_worker_fallback(dr, monkeypatch, has_ledger):
    ...
    def invoke(messages):
        ...
        content = 'A sufficiently long actor dossier with preserved evidence.'
        if has_ledger:
    ...
    # Main's bounded ledger-format repair must retain ASTRA's evidence boundary
    # on both the original synthesis and the optional retry.
    assert len(captured) == (1 if has_ledger else 2)
