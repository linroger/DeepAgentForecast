...
def test_sync_preserves_linear_engine_and_compaction_safety(tmp_path, monkeypatch):
    """Both sides of the main/ASTRA merge must reach the child deployment."""
    bridge_dir = tmp_path / "deerflow_bridge"
    deployed_dir = tmp_path / "deer-flow"
    _write(bridge_dir / "deerflow_research.py", "# current bridge\n")
    _write(deployed_dir / "deerflow_research.py", "# current bridge\n")
    modules = {
        "linear_research.py": "# linear research engine\n",
        "research_compaction.py": "# durable compaction receipt verifier\n",
    }
    for name, content in modules.items():
        _write(bridge_dir / name, content)
        _write(deployed_dir / name, "# stale\n")
    monkeypatch.setattr(
        "app.services.pipeline_orchestrator.__file__",
    ...
    for name, content in modules.items():
        assert (deployed_dir / name).read_text() == content
