...
- **Merge rules the engine applies** (know them so you don't fight them): a non-canonical surface form merges INTO a canonical when names match exactly or by containment AND an embedding-cosine gate passes; **two canonical actors are never merged into each other**; the canonical name always survives as the merged node's name; every merge is written to an audit log (`entity_merges.json`) for rollback.
