    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <!-- VIZ-GALLERY：报告完成后展示 viz_manifest 图表画廊。
               无清单（旧报告 404 / 空清单 / 生成期 409）→ 整块隐藏，不产生错误噪音。 -->
          <div v-if="isComplete && vizGalleryCards.length" class="viz-gallery-block">
...
import { getAgentLog, getConsoleLog, getVizManifest, reportAssetUrl } from '../api/report'
...
// VIZ-GALLERY State（viz_manifest 驱动的完成态图表画廊）
const vizGalleryItems = ref([])
const vizSkipped = ref([])
let vizManifestToken = 0

// 拉取 viz_manifest：老报告无清单（404）/ 生成期发布门未过（409）/ 网络错误
// → 一律置空并静默隐藏画廊（degrade-safe，无错误噪音）。
...
// skipped[] → 单行低调脚注（如：3 项图表未达渲染条件，已在正文以表格呈现）。
...
const renderMarkdown = (content) => {
  ...
  // VIZ-GALLERY 缓解：本渲染器不支持图片语法，成稿注入的图表行
  // （<!-- viz:… --> 标记与 ![…](charts/…) 图片）会以字面文本漏出 →
  // 整行剥离，图表统一由完成态画廊展示（surgical，其余渲染行为不变）。
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // VIZ-GALLERY：成稿落地后清单才过发布门 → 完成时刻重拉一次
            loadVizGallery()
...
watch(() => props.reportId, (newId) => {
  if (newId) {
    ...
    startPolling()
    // VIZ-GALLERY：报告 id 就绪即拉清单（生成期 409 → 静默隐藏，完成时会重拉）
    loadVizGallery()
  }
}, { immediate: true })
</script>
...
/* VIZ-GALLERY：完成态图表画廊（与报告正文同一灰阶/衬线词汇表） */
.viz-gallery-block {
  margin-top: 40px;
}
