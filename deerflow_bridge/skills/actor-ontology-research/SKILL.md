...
Output a salience **tier (high / medium / low)** per actor, with the basis. The downstream cap keeps the highest-salience actors; **do not let a well-covered amplifier outrank a pivotal principal.** Aim for roughly **8–20 cast members** for most questions (fewer for narrow ones, up to ~35 for sprawling multi-party situations) — chosen by salience, not by how often a name appeared.
