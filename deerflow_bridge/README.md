...
| `patches/apply_subagent_overlays.py` | Idempotently wraps the installed `SubagentExecutor._aexecute` with a shared lifecycle lease while preserving the vendor version's tracing/session/callback behavior. This makes `RESEARCH_GLOBAL_SUBAGENT_CAP` application-wide across simultaneous lead streams, outer tracks, and processes. | narrow transform of `deer-flow/backend/packages/harness/deerflow/subagents/executor.py` |
| `patches/apply_lead_agent_overlays.py` | Narrow idempotent factory transform that forwards `trim_tokens_to_summarize: null` explicitly. Without it upstream omits the kwarg and silently falls back to LangChain's 4K tail-only summary input. | narrow transform of `deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py` |
