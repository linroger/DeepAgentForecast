...
# macOS Keychain service name written by the Claude Code CLI. On macOS the CLI
# stores its OAuth credential in the login keychain under this generic-password
# service instead of a plaintext ~/.claude/.credentials.json file.
_MACOS_KEYCHAIN_SERVICE = "Claude Code-credentials"


def _load_claude_code_from_macos_keychain() -> ClaudeCodeCredential | None:
    """Read the Claude Code OAuth credential from the macOS login Keychain.

    The Claude Code CLI on macOS persists its credential as a generic-password
    item (service ``Claude Code-credentials``) rather than a plaintext file, so
    the file/env lookups miss it entirely. We shell out to the ``security`` CLI
    to fetch the JSON blob, which the CLI keeps refreshed.

    Returns None (never raises) on any non-macOS platform, missing item, denied
    keychain access, malformed JSON, or expired token, so callers fall through
    to the existing "no credential" error path cleanly. A short timeout guards
    against a blocking keychain-access GUI prompt hanging a headless run.
    """
    ...
    try:
        ...
        logger.debug(f"macOS keychain lookup failed ({type(e).__name__}): {e}")
        return None

    if result.returncode != 0:
        logger.debug(f"macOS keychain has no '{_MACOS_KEYCHAIN_SERVICE}' item (rc={result.returncode})")
        return None
    ...
    try:
        ...
        logger.warning(f"macOS keychain credential is not valid JSON: {e}")
        return None

    return _extract_claude_code_credential(data, "claude-cli-keychain")
...
def load_claude_code_credential() -> ClaudeCodeCredential | None:
    ...
    # Final fallback: macOS Keychain (the Claude Code CLI's default store on macOS,
    # where there is no plaintext ~/.claude/.credentials.json file).
    keychain_cred = _load_claude_code_from_macos_keychain()
    if keychain_cred:
        logger.info(f"Loaded Claude Code OAuth credential from macOS keychain (expires_at={keychain_cred.expires_at})")
