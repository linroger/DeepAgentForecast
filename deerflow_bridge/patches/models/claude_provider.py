...
# 0.5 splits the budget evenly — ample reasoning depth while keeping the long-form
# research dossier / synthesis report from being truncated.
...
class ClaudeChatModel(ChatAnthropic):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        # Prefer the Claude Code OAuth subscription credential whenever one is available
        # AND the currently-resolved key is not already an OAuth token. ClaudeChatModel is
        # the OAuth-first "Claude Code plan" provider, so an ambient/stray ANTHROPIC_API_KEY
        # — e.g. a plain `sk-ant-api03-…` key inherited from a parent-directory `.env` that
        # python-dotenv auto-discovers — must NOT silently shadow the subscription token and
        # cause `401 invalid x-api-key`. If the user is not signed into Claude Code (no OAuth
        # credential found), the resolved API key is kept as-is so explicit API-key use still
        # works.
        _placeholder_keys = ("your-anthropic-api-key",)
        if not is_oauth_token(current_key):
            cred = load_claude_code_credential()
            if cred:
                if current_key and current_key not in _placeholder_keys:
                    logger.info(
                        f"Preferring Claude Code OAuth credential (source: {cred.source}) over an "
                        f"ambient non-OAuth ANTHROPIC_API_KEY (avoids stray-key 401s)."
                    )
                else:
                    logger.info(f"Using Claude Code CLI credential (source: {cred.source})")
                current_key = cred.access_token
            elif not current_key or current_key in _placeholder_keys:
                logger.warning("No Anthropic API key or Claude Code OAuth credential found.")
