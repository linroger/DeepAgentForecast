...
class LoopDetectionMiddleware(AgentMiddleware[AgentState]):
    ...
    def _reset_run_scoped_loop_state(self, runtime: Runtime) -> None:
        """Start each agent run with a fresh loop-detection budget.

        Loop detection guards a single run's behavior ("you are repeating
        yourself" / "N calls without producing a final answer"). The
        per-thread hash window and per-tool frequency counters used to
        accumulate across runs on the same thread, so multi-turn threads
        (multi-pass deep research, or simply a long chat) would permanently
        force-stop a tool type once the *cumulative* count crossed the
        limit: every later turn instantly degraded to "produce final answer
        with results collected so far" even though each individual run was
        well within budget. Resetting at run start keeps the full in-run
        protection while matching the warning text's own semantics.
        """
        thread_id = self._get_thread_id(runtime)
        with self._lock:
    ...
    def before_agent(self, state: AgentState, runtime: Runtime) -> dict | None:
        self._reset_run_scoped_loop_state(runtime)
