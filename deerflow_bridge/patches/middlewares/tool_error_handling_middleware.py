...
def _build_runtime_middlewares(
    *,
    ...
    from deerflow.agents.middlewares.model_concurrency_middleware import ModelConcurrencyMiddleware
    from deerflow.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from deerflow.agents.middlewares.tool_output_budget_middleware import ToolOutputBudgetMiddleware
    from deerflow.sandbox.middleware import SandboxMiddleware

    # Ordering matters: LangChain composes the first wrapper outermost. Retry
    # handling must therefore sit outside the permit, while request-transform
    # middleware sits before the innermost provider boundary. Each actual retry
    # acquires a fresh permit and backoff sleeps hold no scarce capacity.
    ...
    # Last model wrapper = exact provider-attempt boundary after all request
    # transformations and inside LLMErrorHandlingMiddleware's retry loop.
    middlewares.append(ModelConcurrencyMiddleware())
