...
DRF overlay: with an active agentic research workspace, archive full results
before native offloading, and use only archive-backed previews on disk failure.
Archive failures are typed terminal compaction stops, never truncation fallbacks.
"""
...
def _try_native_externalize(externalizer, *args, **kwargs):
    """Only used after a durable archive exists; keep native storage best-effort."""
    try:
        return externalizer(*args, **kwargs)
    except Exception:
        logger.warning("Native externalization failed; using verified research archive")
        return None
...
def _budget_content(
    ...
    *,
    ...
    archive = _active_research_archive()
    archive_ref = None
    if archive is not None and (
        (threshold > 0 and len(content) > threshold)
        or (config.fallback_max_chars > 0 and len(content) > config.fallback_max_chars)
    ):
        archive_ref = archive.archive_tool_result(
            content if archive_content is None else archive_content, tool_name, tool_call_id,
        )
    preview_budget = max(1000, config.preview_head_chars + config.preview_tail_chars + 800)
    if config.fallback_max_chars > 0:
        preview_budget = min(preview_budget, config.fallback_max_chars)
    ...
    if threshold > 0 and len(content) > threshold:
        ...
        if sandbox is not None:
            ...
            if provider is not None and getattr(provider, "uses_thread_data_mounts", False):
                ...
                virtual_path = sandbox_externalize(
                    content,
                    tool_name=tool_name,
                    tool_call_id=tool_call_id,
                    storage_subdir=config.storage_subdir,
        ...
        if virtual_path is not None:
            ...
            if archive_ref is not None:
                native_note = f"\n[Native copy: {virtual_path}]\n"
                return native_note + archive.evidence_preview(
                    archive_ref, max_chars=preview_budget - len(native_note.encode("utf-8")),
                )
    ...
    if archive_ref is not None:
        return archive.evidence_preview(archive_ref, max_chars=preview_budget)
...
def _patch_tool_message(
    ...
    if tool_name in config.exempt_tools:
        return msg
    if tool_name == "read_evidence" and _active_research_archive() is not None:
        return msg  # ContextPolicy already bounds recall; avoid rearchive loops.
...
def _tool_message_over_budget(msg: ToolMessage, config: ToolOutputConfig) -> bool:
    ...
    if (msg.name or "") in config.exempt_tools:
        return False
    if msg.name == "read_evidence" and _active_research_archive() is not None:
        return False
