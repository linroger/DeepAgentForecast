    ...
    configureServer(server) {
      server.middlewares.use('/__export', async (request, response) => {
        ...
        try {
          ...
          await Promise.all([
            fs.writeFile(
              path.join(outputDir, 'deepresearchforecast-system-architecture.tldr'),
              Buffer.from(payload.tldrBase64, 'base64'),
            ),
            fs.writeFile(
              path.join(outputDir, 'deepresearchforecast-system-architecture.svg'),
              payload.svg,
              'utf8',
            ),
            fs.writeFile(
              path.join(outputDir, 'deepresearchforecast-system-architecture.png'),
              Buffer.from(payload.pngBase64, 'base64'),
            ),
            fs.writeFile(
              path.join(outputDir, 'deepresearchforecast-system-render-metadata.json'),
