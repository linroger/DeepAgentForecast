...
### ASTRA-03 implementation slice — 2026-09-08

Implement durable launch-intent admission across API, orchestrator, and browser. Preserve the unkeyed API for existing callers. The browser writes an immutable payload and random intent before its one-shot POST; cross-tab coordination protects storage updates and a read-only lookup reconciles response loss/reload. Keep resolved intents until an explicit New action and never replace an unknown intent because form fields changed.

Use a SQLite ledger under the pipeline data root, outside individual pipeline directories, with a unique intent-key hash, canonical request hash, pipeline identity, initial snapshot and dispatch state. Admit before thread creation; duplicates never mint another identity or reclaim uncertain dispatch. Read current pipeline state for successful replays, including after backend restart or terminal completion. A missing/corrupt/deleted state or interrupted dispatch retains a known identity and exposes an explicit unavailable/recovery state. Keep startup recovery and intentional resume separate from request replay; no new automatic paid recovery loop.

Acceptance: canonical same-key replay and conflict on changed request; concurrent threads/processes admit once; response loss then reload/check sends no new POST; two tabs sharing an intent reuse the same identity; storage failure starts no work; crash/failure before and after state persistence does not redispatch on replay; deleting/corrupting a pipeline cannot resurrect it through a key; distinct intentional keys still work; disabled/currently failing preflight cannot hide an already admitted pipeline. Run isolated backend regression tests, Node unit tests, frontend build and stubbed real-browser scenarios. Preserve original ResearchView cleanup hunks. Independent review precedes commit.

Review-driven extension: support explicit atomic abandonment of an unadmitted intent, retaining a permanent tombstone and returning an already-admitted identity unchanged. This lets users correct rejected payloads without mistaking400/404 responses for authority to forget an in-flight request. Acceptance includes reserve/abandon races, late requests, response loss, and stale browser tabs. Known interrupted admission permits explicit same-ID viewing and deliberate New with confirmation; original dispatch uncertainty remains visible.


### ASTRA-04 accounting foundation — authorized continuation 2026-09-07T17:14:41.655435+00:00

Review saved-run aggregates and bounded logs before selecting changes. Keep runtime artifacts read-only. Implement a durable operation/snapshot ledger before claiming cross-process budget authority: stable run/attempt/source/operation identity, transactional high-water updates, duplicate/stale delivery handling, explicit legacy baseline provenance, and separate attempt/cumulative views. Integrate parent research/simulation imports with stable identities and fix nested simulation chat_json/chat double counting. Repeated telemetry flushes must not add a snapshot twice. Resume must preserve previous accounted spend. Keep unknown usage and estimated/API/subscription billing bases explicit.

Acceptance includes concurrent processes, repeated and growing snapshots, replay after parent restart, failure between ledger and compatibility projection, stable historical-baseline import, no same-attempt double flush, nested versus standalone simulation methods, and offline original-run arithmetic. Independently review and run affected tests before commit. Provider-boundary reservations and live child coverage remain an explicit ASTRA-04 subtask until separately implemented and verified; do not mark the full recommendation complete on accounting tests alone.

- 2026-09-07T18:00:54.750363+00:00: ASTRA-04a complete offline with 532 passing tests, native browser acceptance, benchmark and independent review. See docs/research/astra-usage-verification.json. ASTRA-04b remains pending; full cumulative budget coverage is not claimed.

### ASTRA-04b1 research-stream accounting — authorized continuation 2026-09-07T18:09:07.551426+00:00

Benefit hypothesis: eliminate replayed checkpoint/history usage and recover later same-message growth and pre-interruption observations in research, the dominant historical token stage. Preserve generated content, tool/source receipts, actor contracts, checkpoint identity, and existing model-concurrency permits. No provider calls or runtime sync are authorized for validation.

Implement only this producer/consumer boundary: seed per-stream usage high-water marks from pre-invocation checkpoint messages; account values-mode usage before display-only seen-message filtering; retain consistent positive deltas; emit stable-schema incremental usage observations so the bridge can log observed consumption even if no end event arrives; suppress the matching final aggregate from being charged again. Keep legacy end-only clients compatible and unknown baselines/provenance explicit. Validate behavior using the complete patched stream generator with an offline agent/checkpointer, then the real bridge consumer and parent durable meter. Test repeated/full-history messages, same-ID growth, stream modes, pass/resume replay, interruption/corrective break, malformed/drifted patch targets, and source/deployed separation. Independent review and an owned commit follow passing gates.

Later ASTRA-04b prerequisites remain open: per-physical backend API attempt accounting before content validation, correct fast-provider attribution, explicit SDK retry ownership, then transactional reservation/settlement with unresolved holds. Estimates must not be represented as hard input or invoice bounds; CLI currently ignores max_tokens and needs a separate bounded design. Preserve caps0 as unlimited.

- 2026-09-07T18:32:47.705762+00:00: ASTRA-04b1 complete offline with577 backend+98 overlay tests and23 upgrade checks;2 deployment-only skips. Numeric full-client reproduction, permanent compaction/interrupt/source-boundary tests, independent review and source hashes are recorded in docs/research/astra-research-stream-verification.json. Next authorized slice is04b2 before reservations.

### ASTRA-04b2 physical API attempts — authorized continuation 2026-09-07T19:09:16.153561+00:00

Benefit hypothesis: successful-only outer metering loses usage from billed empty responses and retries, and SDK retry defaults can multiply the application's configured attempts when HTTP/2 construction falls back. Record each OpenAI-compatible SDK attempt at one shared boundary before validating response content; resolve actual provider/model, disable hidden SDK retries consistently, and prevent budget/accounting control exceptions from becoming retries or fallbacks. Keep configured retry counts, fallback availability, model choices, source/actor contracts, and caps0-as-unlimited unchanged.

Use local per-attempt UUID and pinned run/stage attribution. Register a zero-counter in-flight operation before SDK dispatch, settle returned usage (strict counters and explicit source/cache uncertainty) or an unknown outcome on that same operation, then validate content and check the existing recorded-usage budget. Calls that cannot settle retain an explicit in-flight/unknown row; no claim of complete physical or invoice coverage. Preserve last-response compatibility data for the detached simulation wrapper without using it as the authoritative API meter. API missing-usage estimates, if retained for compatibility, must be labeled estimates rather than known zero. Keep CLI accounting behavior separate and full reservations pending.

Acceptance: empty response→success sums both usage reports; all failures and failed-primary→fallback retain separate physical identities; JSON repair counts both calls once; native tools share the boundary; fast routing attributes/prices/options to the provider actually selected; no stale usage after a missing report; direct and shared SDK clients have zero hidden retries even with HTTP/2 disabled; local httpx.MockTransport counts actual transmissions without network; concurrent use does not cross usage identities; storage/budget failures admit no retry/fallback. Test actual SDK responses and malformed/missing counters, checkpointed operation outcomes, and inferred-context attribution stability. Independent review and affected backend regressions precede commit.

- 2026-09-07T19:29:32.165379+00:00: ASTRA-04b2 verified offline: 741 backend tests, one deployment-only skip, independent review and real-SDK comparison. See docs/research/astra-api-attempt-verification.json. Full ASTRA-04 stays open; ASTRA-04b3 will expose unresolved operations and their restart/admission policy before reservations.

### ASTRA-04b3 unresolved operation visibility and admission — authorized continuation 2026-09-07T20:10:06.853947+00:00

Benefit hypothesis: a crash after the durable dispatch marker must not make uncertain provider work look like a clean zero on restart; a malformed provider usage stop must survive process reset. Reuse the existing usage_operations table and keep usage-ledger/v1 compatible. Add a compact run-wide api_operation_state projection in the same read transaction as totals, with in_flight, unknown and accounting_error counts and optional current/other owner counts keyed by an explicit current_attempt_id. Attempt token totals keep existing delta attribution semantics. No owner lists, private payloads, invented consumption or reservations.

1. Add indexed API uncertainty queries. New dispatch blocks accounting_error from any owner and in_flight from another owner; same-owner concurrent work and unknown transport errors remain admissible. Read/attach/late settlement stay available. A separate completion check rejects any in_flight or accounting_error. Operation uncertainty uses a typed recoverable accounting exception without poisoning the storage-failure latch. Keep durable storage failures sticky.
2. Persist malformed provider usage as accounting_error, distinct from retryable transport unknown. Physical-operation replay cannot regress completed/unknown/error statuses into an in_flight marker; status settlement must remain monotonic without disturbing growing research/simulation snapshots. Precise known recovery may settle the same operation; no automatic unknown release, timeout forgery or administrative reset endpoint is added.
3. Primary wires pipeline start, full/research-only completion and status/telemetry JSON. Recorded totals remain visible; a blocked admission exposes a stable reason and unavailable budget remainder. A status request with no current attempt identity must leave current/other counts unknown rather than infer liveness. No frontend implementation in this slice.
4. Verify zero-counter crash persistence via a separate process and offline SDK dispatch: zero new provider calls after another-owner unresolved marker or durable accounting_error; current-owner concurrency, ordinary429 retry, late settlement and exact-once token growth continue. Test JSON/status projections, full/research-only completion, old-schema read-only compatibility and query scaling. Compare the last committed baseline with current code using local fixtures only. Run affected gates, independent review and commit owned files.

Limits: this is a visibility/recovery admission prerequisite; no physical hard spend cap, concurrent reservation, child-wide provenance, leases or mathematical Pareto optimality claim. An in-flight marker can also represent a crash before dispatch, so blocking trades availability for retained uncertainty until exact evidence settles it. Another owner can still be alive; a different attempt ID is not proof of death. Caps0 remains unlimited with respect to spending, while accounting-integrity checks remain active.

- 2026-09-07T20:19:11.013964+00:00: Refined the policy/storage distinction: successfully committed accounting_error is recoverable by precise same-operation settlement even in the originating binding; failure to persist that observation remains sticky. New dispatch-marker insertion repeats the indexed admission check in its BEGIN IMMEDIATE transaction to prevent cross-owner admission races. These are accounting-integrity controls, not token reservations.

- 2026-09-07T20:26:09.611959+00:00: ASTRA-04b3 complete offline: 782 tests, one deployment-only skip, indexed comparison and independent review. See docs/research/astra-unresolved-verification.json. The next accounting dependency is ASTRA-04b4 detached simulation physical transport; broader reservations remain pending.

### ASTRA-04b4 detached simulation accounting — authorized continuation 2026-09-07T21:14:45.416873+00:00

The current success-only child aggregate loses physical retry/failure observations already captured by LLMClient, and direct CAMEL uses a separate SDK path. Bind explicitly launched children to the existing parent ledger and the **same parent attempt identity**; concurrent child work therefore remains current-owner work, while a parent restart sees unresolved old-owner operations. A new launch token identifies each child process without changing accounting ownership. Validate existing storage and simulation/run lineage before model work; never silently initialize a replacement parent ledger or inherit an unrelated ambient binding. All three simulation entrypoints need the same bootstrap.

Capture direct CAMEL non-streaming SDK attempts at the HTTP send boundary, before SDK parsing, preserving existing SDK retry policy and actual model/provider attribution. Each send gets an in-flight marker and settles its own usage or explicit unknown/error state; underlying LLMClient fallback remains independently metered. Keep native tools and structured parse coverage, disallow unsupported streaming in the explicitly accounted contract, and avoid recording synthetic fallback/CLI completions again. CLI successful logical text estimates remain incomplete physical coverage.

The compatibility child JSON remains diagnostic. New child launches persist a parent-owned lineage receipt before dispatch, and only a matching accounted snapshot bypasses legacy aggregate import. Legacy/unbound snapshots retain current high-water migration behavior. Durable usage survives abrupt child exit before JSON flush and is immediately available to parent cumulative budget checks. Preserve source/actor seals, provider routing and historical artifacts. No hard reservation or complete billing claim.

Acceptance: actual SDK sync/async MockTransport retry, tools, parser error, empty-response usage and fallback deduplication; parent/child process sharing with immediate totals, abrupt exit and other-owner restart stop; repeated compatibility import does not double-credit; invalid/missing lineage/storage fails before provider work; all three entrypoints bootstrap, and unrelated launch environments remain unbound. Measure local instrumentation overhead and compare baseline retry/empty/crash accounting with fixtures. Run affected backend gates, independent review and owned commits.

- 2026-09-07T21:36:44.479039+00:00: ASTRA-04b4 complete offline with960 tests, one deployment-only skip and69 independent focused cases. Review refined completion from process-local tracking to durable launch-prefixed API identities; the transactional query permits sibling scopes and rejects own unfinished work without a commit-return window or terminal replay mismatch. Known shared simulations cannot downgrade to legacy snapshots by losing token/authority. See docs/research/astra-simulation-verification.json. Next04b5 begins with cumulative-budget scaling measurements before a bounded reservation design; all broader limits remain explicit.

### ASTRA-04b5a narrow cumulative budget projection — authorized continuation 2026-09-07T22:11:58.733583+00:00

Initial local fixture measurements at100/1,000/10,000/100,000 delta rows showed full grouped snapshot medians0.45/2.16/21.26/277.68ms versus simple two-value query0.14/0.48/3.81/59.53ms. These are preliminary warm local read measurements, not production latency; recorded saved aggregate counts cannot establish historical physical ledger sizes. Budget limits default to0, so benefits apply only when a guard is enabled.

Benefit hypothesis: the guard needs cumulative total tokens and optionally cost, but currently pays for all10 metrics,8-dimensional grouping, multiple Python breakdowns and unresolved state aggregation on every check. Add an existing-storage read-only budget_totals method and LLMMeter adapter. Token-only checks use a scalar integer SUM; cost-enabled checks retain the existing eight-dimensional grouping, grouped accumulation order and six-decimal rounding so exact threshold behavior does not change. Preserve strict greater-than caps, unlimited early return, cumulative legacy/replay/resume totals, explicit run attribution and fail-closed storage/latch behavior. Full snapshots/status/admission checks stay authoritative for their existing uses.

No schema, new index, materialized total or write-path change in this slice. This deliberately leaves the residual scan O(number of run deltas), avoids migration/mixed-writer/rounding risk, and makes the measured benefit independently reviewable before reservations. Keep ASTRA-04b5 partial:04b5a is budget-read scaling;04b5b will implement a bounded transactional reservation/settlement design with explicit uncertainty.

Worker owns usage_ledger.py, telemetry.py and focused budget-total tests. Root owns independent caller-level acceptance, comparative harness, current-source/saved evidence, documentation and integration. Acceptance covers legacy import, exact/stale/growing snapshots, resumed/child writes, exact token/USD thresholds and float rounding boundaries, empty/missing/corrupt storage, no read with limits0, independence from full projection/unresolved count aggregation, and actual provider-boundary stops. Compare pinned baseline and current methods over100/1,000/10,000/100,000 rows with all fixtures/config/timings labeled. Require affected offline gates, independent code/evidence review and an owned commit.

- 2026-09-07T22:25:48.747239+00:00: ASTRA-04b5a complete offline:998 tests, one deployment-only skip, five Python files clean and38 independent cases. Review added an exact integer-overflow fallback inside the existing read transaction. Final token-only10000-row median21.6447→3.6771ms and cost-enabled21.6541→15.6073ms are local enabled-guard improvements, not production effects. See docs/research/astra-budget-verification.json. Full04b5 remains open; next04b5b is a bounded transactional reservation/settlement design with explicit estimation and uncertainty.

### ASTRA-04b5b1 transactional token reservations — authorized continuation 2026-09-07T23:13:40.833894+00:00

Benefit hypothesis: serializing planned token exposure and the existing physical dispatch marker prevents concurrent API calls from spending the same recorded remainder. This is a token-planning policy for explicitly bound durable API work when LLM_RUN_BUDGET_TOKENS is positive. Caps0 and USD-only guards retain their existing behavior; monetary reservations, CLI physical bounds, research child coverage and hard invoice/input guarantees remain separate work.

Reserve the estimated serialized text input plus an explicit positive output cap before each physical dispatch, in the same BEGIN IMMEDIATE transaction as its zero-counter marker. Include messages, tool/function definitions, tool choices and structured-output schema in a versioned UTF-8-byte/4 estimate; persist only numbers and estimator identity. Reject ambiguous/missing output caps, multiple completions and unsupported nontext or hosted-tool input before dispatch rather than injecting an output limit or changing quality settings.

Use additive durable reservation and per-run policy tables, without altering usage_operations columns or existing usage delta authority. Pin the enabled token policy and require matching per-operation reservations at the database boundary so old marker writers cannot bypass it. Admission requires recorded cumulative tokens plus all outstanding holds plus the new planned allowance to fit the pinned cap. Settlement and high-water/delta updates share one transaction. Precise known terminal usage releases the hold; unknown, missing, malformed or estimated usage retains its unobserved allowance. A held allowance is never actual consumption, and planned input is an estimate rather than an upper bound. Reject unsafe activation with existing unresolved unreserved API work; no expiry releases uncertainty.

Acceptance: competing same-owner threads/processes admit only fitting work; exact threshold fits; settled low usage releases only unused allowance; unknown outcomes survive retry/restart and cannot disappear; failed settlement rolls back both counters and hold; mixed older marker writers fail before dispatch; all actual LLMClient plain/tool and native sync/async boundaries obey policy; default-disabled behavior and actual usage projections remain intact. Compare before/after admitted transmissions and local overhead with offline transports, run affected accounting/pipeline gates, independently review code and evidence, then commit one coherent slice. Full04b5b stays partial for remaining monetary/coverage boundaries.

- 2026-09-07T23:33:46.182071+00:00: ASTRA-04b5b1 verified offline with1072 passing tests and one deployment-only skip. Finalreview74 newcases clear; native/sharedSDK, transaction/process, request-copy, captured-cap and status boundaries pass. The source-backed follow-up04b5b2 is an explicit native output-cap configuration path: the current production factory omits caps, so token-enabled uncapped requests correctly stop before send. Keep that default and add opt-in configuration in the next coherent slice. Monetary admission remains04b5b3. See docs/research/astra-reservation-verification.json.

### ASTRA-04b5b2 explicit native output policy — authorized continuation 2026-09-08T00:16:42.855395+00:00

Benefit hypothesis: configured native simulation output limits should reach actual SDK requests, allowing the existing token reservation boundary to admit fitting work. Add OASIS_MAX_OUTPUT_TOKENS (default0, disabled) and OASIS_OUTPUT_TOKEN_PARAMETER (defaultmax_tokens; explicit max_completion_tokens alternative). Do not guess provider/model support or change uncapped defaults, routing, reasoning, source/actor seals or CLI behavior. Direct native and boost routes share the same output policy. Native fallback carries the numerical cap through the existing LLMClient max_tokens argument.

Use a strict metadata-only policy object with schema oasis-output-policy/v1, max_output_tokens as a nonnegative integer no larger than2**53, and parameter from the two supported names. A pure validator returns an independent copy. New parent-bound launch contexts always carry native_output_policy, including disabled0; their saved authority also binds the existing token/USD budgets. Old v1 contexts retain their five-field authority and resolve to a fixed disabled legacy policy. Missing, injected or altered new policy must fail against saved authority. Set the child environment before Config import, then let bound factories read the validated context instead of ambient configuration. Preserve simulation_config.json bytes and existing seals.

Resume decision: within a parent pipeline, subsequent launches of the same simulation must match any previously recorded native output policy. A conflicting current setting is rejected before a new launch is saved, so checkpoint resume cannot silently change this quality setting. The first policy-aware launch of legacy history captures current explicit configuration; later launches enforce it. Standalone unbound factories use validated Config and do not acquire a cross-process resume guarantee.

Worker owns config.py, oasis_output_policy.py, oasis_llm.py, .env.example and focused factory tests. Root owns simulation_usage.py, parent launch authority, child/process/resume acceptance, comparative evidence and documentation. Acceptance covers default0 byte-preserving requests, both parameter names through actual factory sync/async create and parse, general/boost routing, numeric fallback caps, malformed settings before client creation, bounded child dispatch into the shared reservation ledger, ambient drift, receipt tamper/removal/injection, unchanged sealed config, resumed-policy mismatch, CLI bypass, and final affected gates plus independent review. Compare the pinned factory baseline with current behavior using offline SDK transport, not paid runs; record local overhead and unmeasured production effects.
...
- 2026-09-08T00:36:08.402049+00:00: ASTRA-04b5b2 complete offline with 1,165 passing tests, one deployment-only skip and 67 independently reviewed new cases. Factory request adapters preserve CAMEL memory; policy/budget launch authority, persisted resume and real actor seals pass. See docs/research/astra-native-cap-verification.json. Monetary reservation policy 04b5b3 is next; full ASTRA-04b5b remains partial.

### ASTRA-04b5b3a captured API pricing prerequisite — authorized continuation 2026-09-08T01:17:55.439536+00:00

Current source and offline reproduction show that an unpriced API route records positive known usage at zero cost and passes a dollar guard. Replaying unchanged usage after changing configured rates adds cost-only high-water growth. Provider-wide rough rates, permissive overrides and missing cache billing do not justify monetary holds yet. Split04b5b3 into this captured-pricing prerequisite and04b5b3b monetary admission; do not mark the parent complete.

Benefit hypothesis: capture a strict metadata-only price estimate once before each physical API dispatch, persist it with the existing operation, and reuse it for every settlement/replay. Distinguish unpriced(null rates) from explicit zero rates. Extend existing LLM_COST_PER_MTOK resolution with exact provider:model overrides before provider-wide overrides, then existing rough builtins. Model keys preserve model case; provider keys normalize lower case. Validate complete JSON mapping and exact two-number finite nonnegative pairs; malformed configuration produces a recoverable predispatch BudgetExceeded, not a sticky storage failure. Default empty configuration keeps existing valid rate arithmetic and zero/unlimited behavior; missing or placeholder-zero builtin pricing remains an explicit unpriced quote and cannot admit work under a positive dollar cap. No endpoint/model prices are invented or fetched.

Use api-cost-quote/v1 with exact fields schema, provider, model, source, rate_key, input_usd_per_1k, output_usd_per_1k. Source is configured_model, configured_provider, builtin_provider or unpriced. Quote-derived costs remain estimates, calculated using inclusive input with no inferred cache discount and existing float order. Persist quote in usage_operations metadata_json without changing cost grouping/round6 or usage_delta schema. Validate attribution, exact cost calculation and estimated flag; a quoted operation cannot change or lose its quote, nor can a legacy operation acquire one on replay. An additive update trigger prevents older writers from stripping the quote; require the guard for new quoted writes. New older-writer operations and legacy/non-API estimates are not retroactively covered.

Root owns new api_cost.py, shared/native request integration, actual SDK admission/rate-drift/retry/fallback and child acceptance, comparison and documentation. Worker owns usage_ledger.py, telemetry.py and test_api_cost_ledger.py. Independent reviewer owns no files. Acceptance: both API paths reject invalid/unpriced dollar-enabled requests with zero transmissions and no sticky latch; explicitly priced zero remains allowed; two models resolve distinct configured rates; mid-call rate changes and exact replay do not add fictitious cost; quoted metadata cannot be removed/replaced by current or older writers; historical noquote behavior remains compatible; quote costs stay estimated and cache token counts unchanged. Preserve native output policies, token holds, attribution, defaults, seals and routes. Run relevant full gates and offline before/after comparison, audit source/evidence hashes, commit one coherent slice. No provider, pipeline, runtime, deployment, secrets or publication retry.

Limits: these are captured estimates, not invoice rates, conservative upper bounds or monetary reservations. Per-operation pricing prevents replay drift but does not yet pin rates/caps for an entire run or recover unknown historical costs. Monetary admission remains pending with an explicit price/coverage policy.

- 2026-09-08T01:41:49.138035+00:00: Implementation refinement and result: the initial metadata-only update guard was insufficient for copied-quote older-writer repricing. Final runtime has quoted INSERT/UPDATE consistency guards and previous-counter formula validation. ASTRA-04b5b3a verified offline: strict captured API estimates now reject unpriced dollar-enabled sends and prevent rate-drift replay inflation. Quoted INSERT/UPDATE consistency guards also reject actual older-writer repricing, and prior-counter validation prevents negative corrections. Final1,266 backend tests pass with one deployment-only skip; all101 new cases independently reviewed; all13 changed Python files pass Ruff/parsing. Paired18-invocation SDK comparison confirms missing-price sends1→0, response estimate0.006→0.003 and exact replay extra cost0.003→0, with identical default wire bytes and costs. These are synthetic accounting outcomes, not savings or invoice accuracy. Evidence: docs/research/astra-api-cost-verification.json and runbook.77 saved artifacts/32 states/12 bounded logs remain unchanged. Hourly loopACTIVE; full04 remains open for monetary holds, rate/cap pinning and historical price coverage04b5b3b. No provider, deployment, saved-run write or rejected-auth retry.

### ASTRA-04b5b3b1 recorded price coverage — authorized continuation 2026-09-08T02:22:18.206623+00:00

Current source and real offline SDK reproductions show two distinct gaps: concurrently admitted calls can share a dollar remainder, and enabling a dollar limit after an unpriced completed operation admits a newly priced request against a numeric zero historical cost. Child price settings and caps are not run-wide pinned. Full monetary reservations also cannot be bolted independently onto token holds: token denial currently commits its policy internally, which could commit an orphan dollar hold. Select one prerequisite: expose recorded price coverage and reject currently dollar-enabled covered API admission when prior recorded price evidence is missing. Keep dollar holds, rate/cap pinning and atomic joint capacity planning pending under04b5b3b2.

Benefit hypothesis: explicitly separating missing prices from zero-dollar usage prevents further covered provider work from being admitted against a misleading historical monetary total. Add a run-wide cost_coverage projection to durable snapshots, flushed accounting and public spend status. Count priced/unpriced/unquoted API operations separately from uncertain usage; classify non-API opaque observations, strict zero-token/zero-cost local cache observations and imported legacy baselines explicitly. Counts describe recorded operations, not all physical usage. Empty history means an empty recorded scope. Quote presence does not establish precise consumption; same-owner priced in-flight work and priced-but-inexact usage remain visible and do not independently block this PRICE-only prerequisite. Valid known usage on a terminal HTTP error is not automatically inexact solely because its status is unknown.

Before a new USD-enabled durable API dispatch, require a priced current quote and check prior price gaps inside the marker BEGIN IMMEDIATE transaction, before writing marker, deltas or token holds. Missing/unpriced API quotes, non-API observations except narrowly recognized local cache hits, and any opaque imported baseline block with recoverable BudgetExceeded. Previously unpriced quotes stay immutable; changing rates later cannot rewrite historical receipt semantics. Rejected calls leave no new work and do not poison storage state. USD0 and unbound legacy paths preserve behavior; no run-wide USD-policy or older-writer dispatch fence is claimed. Read-only coverage remains available without schema migration. An additive builtin-SQL expression index shares exact classification expressions with coverage grouping and gap existence queries; old writers maintain it automatically. Validate quote shape, attribution, rate keys and numeric/null partition before considering prices available. Malformed records fail closed or classify as missing/invalid price evidence.

Worker owns usage_ledger.py, telemetry.py and new test_cost_coverage_ledger.py. Root owns capture of per-request dollar-enabled decision in api_cost.py/shared/native boundaries, orchestrator flush/status propagation, integration/recovery/child tests and final evidence. A reviewer owns no code. Forensics/comparison specialist owns sanitized saved evidence and a paired SDK/index-read benchmark. Acceptance covers prior unpriced and legacy gap denial, explicit zero, exact native failed-response usage, cache-only distinction, no implicit serialization of same-owner priced work, snapshot/read-only/restart consistency, marker race/no orphan token holds, actual older-writer index maintenance, corrupted quote coverage, configuration restoration without reset and current round6 thresholds. Measure query plan/local scaling separately from synthetic avoided transmissions; retain no-production-speedup and no-invoice claims. Run relevant affected gates and independent source/evidence review before one coherent owned commit. No provider, pipeline, runtime, deployment, secrets or unchanged rejected-auth retry.
...
### ASTRA-INTEGRATION application context verification — user-directed 2026-09-09T14:41:58.171164+00:00

The user prioritizes checking all components and effortless context transfer from RESEARCH through REPORT. Read the detailed brief and acceptance table in docs/handoff/astra-integration/handoff.md. Trace actual producers/consumers and bounded context, run broad offline backend/frontend/application gates, preserve source/actor contracts and original cleanup ownership, then fix confirmed defects in individually verified slices. Add a source-linked context matrix and integrated scenario evidence. Existing implementation authorization covers repairs; no live provider, pipeline run, deployment or destructive actions are part of this check. Pending monetary reservations resume afterward.

- 2026-09-09T14:48:01.500847+00:00: ASTRA-INTEGRATION first confirmed repair: preserve the existing false-wins access rule at the shared-world selector. Add top-level/qualifier false and conflicting-flag regressions that retain modeler evidence while excluding the denied claim from world brief/config/event prompts and injected agent knowledge. Change only canonical shared-row selection, preserve legacy behavior and current seals; verify with adjacent actor/config tests. Runtime edits wait until the baseline gate finishes.

- 2026-09-11T13:17:49.866526+00:00: Runtime hold lifted for the narrow actor-denial repair after baseline completion and identification of the external launcher error. Shared canonical selection will reject top-level or qualifier literal false before rendering, retain current public controls and modeler audit data, and validate real sealed packs plus six downstream prompt/feed channels. Ontology/report repairs remain dependency-ordered follow-ups.
...
- 2026-09-13T11:37:29.669010+00:00: Independent red evidence extends issue02 to the directly connected graph full-document sanitizer. Use the same explicit no-presentation-cap mode before chunking; maintain finite per-episode envelopes. Accept increased episode count only as restored previously omitted evidence; record local memory/time tradeoffs separately. Ontology reuse03 remains separate.

- 2026-09-13T11:46:52.662469+00:00: Completed the verified full-sanitation slice02 with 12 boundary and 69 independent passes; broad gate3905pass/1known scaffold failure/17skip/11existingxfail. Preserve known failures explicitly. Input binding and dependent ontology reuse03 is the next design; it remains unimplemented.
...
- 2026-09-13T13:10:37.091677+00:00:03a verified with final owner+child proof closure,73focused/82independent passes and broad3923pass/1knownfail/17skip/11existingxfail. Stop-on-invalid-reuse is complete; safe automatic regeneration is intentionally not claimed. Parent03 stays in progress for the documented03b/03c dependency design.

### ASTRA-EXPERIENCE user expansion — 2026-09-16T10:35:18.838753+00:00
The user requests every successful past run audited per stage/log, direct localhost research entry, professional UI, stronger visualization/Polymarket integration and continued workflow optimization. The detailed brief is docs/handoff/astra-experience/handoff.md. Audit independently in parallel; implement one coherent UI entry/workspace slice first with browser-visible hierarchy/navigation/accessibility acceptance and preserved launch-intent/usage contracts. Original cleanup remains intact. Existing integration/accounting work remains queued. The user's implementation request authorizes this incremental work; no paid/live validation or production service restart is required.

- 2026-09-16T10:42:39.852662+00:00: Direct-entry slice also owns the source-proven launcher/proxy mismatch: explicitly share the resolved FLASK_PORT with both processes, disable Vite's independent browser opening, and validate same-origin readiness with isolated launcher fixtures as appropriate. This is part of delivering one working localhost entry, not a separate workflow refactor. UI specialist owns ResearchView/router/preflight; launcher specialist owns Vite config/start script and their focused tests; root owns browser acceptance and docs. No real service restart is needed for implementation.
...
- 2026-09-16T12:30:36.676096+00:00:06a verified with3978backend/144focused/109independent passes andoneknownscaffoldfailure. The indexed final-accounting race is covered. Keep06b durable recovery and06c deadlines open. Next source-only slice is binary-market token identity02, preceding quote-provenance01 and calibration-resolution03.
