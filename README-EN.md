...
# OpenAI-compatible API (only needed when LLM_PROVIDER=openai)
...
> **LLM providers.** By default MiroFish drives every LLM call (ontology extraction,
> persona generation, simulation config, the OASIS agents, and the report agent) through
> the local **Claude Code CLI** (`claude-cli`). Set `LLM_PROVIDER=codex-cli` to use the
> Codex CLI instead, or `LLM_PROVIDER=openai` to fall back to the original OpenAI-compatible
> path (with optional boost config). CLI providers require the corresponding CLI to be
> installed and on your `PATH`; they consume your subscription and need no API key.
> Note: CLI providers proxy through a single-prompt bridge and do **not** support native
> function/tool calling inside OASIS (the report agent already uses prompt-based tools, so it
> is unaffected).
